---
title: Revista Latinoamericana de Recursos Naturales
author: Jospeh Moe
author_title: CiteDrive Bot
author_url: https://citedrive.com/
tags: [author-date, engineering]
---

When you 're tasked by using "Revista Latinoamericana de Recursos Naturales" (Rev. Lat. Rec. Nat.) for your term paper, bachelor 's, or master 's thesis to create citations and bibliographies.
Manually entering bibliographies and in-text citations can be time-consuming and error-prone. And while we will try to present you some examples on how to do the referencing by hand, at CiteDrive, we do it all for you.

## Details  Revista Latinoamericana de Recursos Naturales
* **Title:** Revista Latinoamericana de Recursos Naturales
* **Type:** author-date
* **Short Title (Id):** Rev. Lat. Rec. Nat.)
* fields: , engineering

## Reference tools?
### CiteDrive - citedrive.com
** How can CiteDrive help you with Revista Latinoamericana de Recursos Naturales? **
We designed CiteDrive from the ground up to be as simple and easy to use as possible - you 've got more important things to do than to worry about bibliographies. Simply with CiteDrive, you create a library for your work. Collect all the items you need to cite, and we do the rest. Click on "create bibliography" when you 're ready. We then create citations and the bibliography in Revista Latinoamericana de Recursos Naturales for you. For in-text-citations, simply select one or multiple items and press on the "cite"-button. Add collaborators to your project if you are working with other students on your project. Anyone can make changes at any time.


!["Revista Latinoamericana de Recursos Naturales"](images/revista-latinoamericana-de-recursos-naturales.svg "Choose Revista Latinoamericana de Recursos Naturales in the CiteDrive App")

### Zotero - zotero.com

coming soon

## Example items

This table demonstrates how you can use these elements to create proper in-text citations and the bibliography. The first element has only one author, the second has two, and the third has three. Also, note that the second element has no date.


<table>
  <tr>
   <td>Number
   </td>
   <td>Title
   </td>
   <td>Type
   </td>
   <td>Authors
   </td>
   <td>Date
   </td>
   <td>Journal
   </td>
   <td>Pages
   </td>
   <td>Volume
   </td>
   <td>Issue
   </td>
   <td>Access Date
   </td>
  </tr>
  <tr>
   <td>1
   </td>
   <td>Title #1
   </td>
   <td>Article in Journal
   </td>
   <td><strong>Jane Doe</strong>
   </td>
   <td>2008
   </td>
   <td>XYZ
   </td>
   <td>1-999
   </td>
   <td>3
   </td>
   <td>2
   </td>
   <td>--
   </td>
  </tr>
  <tr>
   <td>
2
   </td>
   <td>Title #2
   </td>
   <td>Website
   </td>
   <td><strong>Jane Doe, </strong>John Doe
   </td>
   <td>---
   </td>
   <td>---
   </td>
   <td>---
   </td>
   <td>4
   </td>
   <td>---
   </td>
   <td>2020-05-01
   </td>
  </tr>
  <tr>
   <td>3
   </td>
   <td>Title #3
   </td>
   <td>Article in Journal
   </td>
   <td>Jane Doe, John Doe, Monty Python
   </td>
   <td>1988
   </td>
   <td>ABC
   </td>
   <td>---
   </td>
   <td>---
   </td>
   <td>16
   </td>
   <td>---
   </td>
  </tr>
</table>



## How do I cite in  Revista Latinoamericana de Recursos Naturales?

Let us produce some example in-text-citations:

:::note Citations


This sentence refers to the first item from the table. (Doe,  2008) This, in turn, refers to the second (Doe &amp; Doe, n.d.)
and then to the third.  (Doe, n.d.)
This paragraph demonstrates the clustering of citations by citing the first and third item. (Doe,  2008; Doe, n.d.)

:::

## How do I create a bibliography in Revista Latinoamericana de Recursos Naturales?

This bibliography contains all items from the sample table. Please note that these examples in the table consist only of articles. Other source types may require different handling, such as web pages that expect an access date, or movies, music tracks etc.

While we would like to show a few such concrete examples in the future - this series of articles is in fact in progress - we would like to point out that CiteDrive already features them.

:::note Bibliography

Doe, J.,  2008. Title #1. <i>Imaginary Journal</i>,  ed., 3: 1–999.

Doe, J. &amp; Doe, J., .n.d.. <i>Title #2</i>.  ed., vol. , vols. ,  p., p. , Available: {"<"}https://example.com/{">"},  [Consulted: May 1, 2021].

Doe, J.; Doe, J. &amp; Python, M., n.d.. Title #3. <i>ABC</i>,  ed..



:::


        